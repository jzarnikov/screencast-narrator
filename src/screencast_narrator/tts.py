...
class KokoroTTS(TTSBackend):
    ...
    def generate(self, text: str, output_path: Path) -> None:
        ...
        cached = self._cache_dir / (self._cache_key(text) + ".wav")

        if cached.exists():
            copy2(cached, output_path)
            return

        self._generate_raw(text, output_path)
        copy2(output_path, cached)

    def _generate_raw(self, text: str, output_path: Path) -> None:
        import kokoro
        import numpy as np
        ...
        all_audio = []
        for _gs, _ps, audio in pipeline(text, voice=self._voice, speed=1.0):
